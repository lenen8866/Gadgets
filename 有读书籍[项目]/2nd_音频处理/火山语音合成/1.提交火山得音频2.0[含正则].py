...
def init_config():
    ...
    try:
        ...
        global appId, accessToken, lang, voiceType, inputPath, outputPath, splitLength
        ...
        splitLength = get_or_default(config, section, "split_length", 1024)
...
def query_task(task_id, file_path):
    progress_bar("正在等待异步返回:" + file_path)
    ...
    result = response.json()
    global task_list, current
    if "code" in result.keys():
        print(f"任务未完成:{result['message']}")
        progress_bar("正在等待异步返回:" + file_path + f"【任务未完成:{result['message']}】")
        ...
        if "audio_url" in result.keys():
            ...
            progress_bar("正在等待异步返回:" + file_path + "【任务未完成】")
            task_list.append({"task_id": task_id, "file_path": file_path})
...
def scan_txt_file(file_name, input_file, output_file):
    # 打开文件
    global task_list, total, current, task_queue
    with open(input_file, 'r', encoding='utf-8') as file:
        # 读取文件内容
        content = file.read()
        content = deal_content(content)
        total += 1
        print("发现文件" + file_name + "   文本长度：" + str(len(content)))
        task_queue.append({'file_name': file_name, 'input_file': input_file,
                           'output_file': output_file})


def process_txt_file(file_name, input_file, output_file):
    # 打开文件
    global task_list, total, current
    with open(input_file, 'r', encoding='utf-8') as file:
        # 读取文件内容
        content = file.read()
        content = deal_content(content)
        if len(content) > int(splitLength):
            if int(splitLength) == -1 and len(content) > 1024:
                pass
            else:
                progress_bar("正在处理:" + file_name)
...
def deal_async_task():
    ...
    while len(task_list) > 0:
        print("正在查询异步任务执行结果，剩余任务数%d" % (len(task_list)))
        v = task_list.pop()
...
def main():
    ...
    for file in os.listdir(input_folder):
        if file.endswith(".txt"):
            ...
            scan_txt_file(file, input_file, output_file)

    choice = input("是否执行?(Y/N)")
    if choice != "Y":
        exit(0)
    print("开始处理")
    t1 = threading.Thread(target=async_run)
    t2 = threading.Thread(target=async_run)
    t1.start()
    t2.start()
    t1.join()
    t2.join()
...
def async_run():
    ...
    try:
        ...
        process_txt_file(v['file_name'], v['input_file'], v['output_file'])
...
def progress_bar(msg=""):
    ...
    sys.stdout.write('\r[%s] %d%%  (%d/%d)%s' % (bar, int(current / total * 100), current, total, msg))
    sys.stdout.flush()
    if total == current:
        print()
